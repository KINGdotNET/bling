...
# docker buildx build --tag bling --file .pi/Dockerfile --platform linux/arm64 --cache-from bling:cache --cache-to bling:cache --load --progress tty .

# save the image to result-rootfs.tar
docker save --output ./.tmp/result-rootfs.tar bling
...
sudo bash -c "echo bling > ./.tmp/result-rootfs/etc/hostname"
...
# Make the image file
fallocate -l 4G "images/bling.img"

# loop-mount the image file so it becomes a disk
sudo losetup --find --show images/bling.img
